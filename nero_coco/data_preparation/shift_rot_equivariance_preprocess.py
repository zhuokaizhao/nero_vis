# the scripts processes the training dataset from COCO datasets for both shift and rotation equivariant experiments
import os
import cv2
import csv
import json
...
def rotate_object(cur_bounding_box, origin, theta):
...
def main():
    ...
    if data_type == 'train' or data_type == 'val' or data_type == 'test':
        ...
        # rotation range used to check to make sure current object supports all possible rotations
        # happened during training (in the augmentation scheme)
        rotation_range = [-180, 180]
        # translation range used to check to make sure current object supports all possible translations
        # happened during training (in the augmentation scheme)
        x_translation = [-64, 64]
        y_translation = [-64, 64]
        ...
        # car, bottle, cup, chair, book image index and bb index
    ...
    if data_type == 'train' or data_type == 'val' or data_type == 'test':
        ...
        print(f'\nGenerating {data_type} dataset for all shift and rotation levels')
        ...
        num_rej_target_no_space_rot = 0
        num_rej_target_no_space_shift = 0
        ...
        for i in tqdm(range(start_index, len(dataset))):
            ...
            for m, k in enumerate(cur_class_ids):
                ...
                if class_names[k] in desired_classes:
                    ...
                    not_qualified = False
                    for rot in range(rotation_range[0], rotation_range[1]):
                        ...
                        if x_min_rotated < 0 or x_max_rotated >= width or y_min_rotated < 0 or y_max_rotated >= height:
                            num_rej_target_no_space_rot += 1
                            not_qualified = True
                            break

                        # move the rotated object to places
                        for x_tran in x_translation:
                            for y_tran in y_translation:
                                x_min_rotated_shifted = center_x_rotated - image_size[1]//2 - x_tran
                                y_min_rotated_shifted = center_y_rotated - image_size[0]//2 - y_tran
                                x_max_rotated_shifted = center_x_rotated + image_size[1]//2 - x_tran
                                y_max_rotated_shifted = center_y_rotated + image_size[0]//2 - y_tran

                                # make sure that the extracted image fits in the original image
                                if x_min_rotated_shifted < 0 or x_max_rotated_shifted >= width or y_min_rotated_shifted < 0 or y_max_rotated_shifted >= height:
                                    num_rej_target_no_space_shift += 1
                                    not_qualified = True
                                    break
        ...
        print(f'target object rejected: no enough area (rot): {num_rej_target_no_space_rot}/{num_target_bb}')
        print(f'target object rejected: no enough area (shift): {num_rej_target_no_space_shift}/{num_target_bb}')
        ...
        np.savez(rejection_path,
                ...
                num_rej_target_no_space_rot=num_rej_target_no_space_rot,
                num_rej_target_no_space_shift=num_rej_target_no_space_shift,
        ...
        labels = 'passed', 'no space (rot)', 'no space (shift)', f'bb too large\n({size_limit*100}% threshold)', 'bb too small\n(1% threshold)'
        sizes = [num_target_passed*100/num_target_bb,
                num_rej_target_no_space_rot*100/num_target_bb,
                num_rej_target_no_space_shift*100/num_target_bb,
